...
// ============ ORG PRESIGNED UPLOADS ============
//
// The multer-based /org/logo and /org/cover endpoints above route the
// file through Express. DigitalOcean App Platform's ingress gateway has
// a ~1MB request body limit that isn't configurable via app.yaml, so
// any image bigger than ~1MB gets rejected with 413 "Content Too Large"
// before it ever reaches multer's 5MB limit. Real phone photos are
// routinely 3-8MB, so the multer path is not viable for end users.
//
// The endpoints below implement the direct browser-to-Spaces pattern:
//
//   1. Client POSTs /api/org/<type>/presign with { contentType }
//      → server returns { uploadUrl, key }
//   2. Client PUTs the file directly to uploadUrl (bypasses our server)
//      → file lands in Spaces, never touches Express or the DO gateway
//   3. Client POSTs /api/org/<type>/confirm with { key }
//      → server validates the key belongs to the caller's org, computes
//        the permanent publicUrl from the Spaces CDN prefix, persists
//        it to orgBranding, returns { url }
//
// The old multer endpoints are left in place for backward compatibility
// with any cached old client bundles. They'll still work for images
// under 1MB. Delete them in a follow-up once logs confirm no legacy
// clients are calling them.
...
/**
 * Shared handler body for logo/cover presign. Generates a key scoped
 * to the caller's org, creates a presigned PUT URL (public-read), and
 * returns both to the client.
 */
async function handlePresign(
  req: Request,
  res: Response,
  prefix: "org-logos" | "org-covers",
  kind: "logo" | "cover"
) {
  try {
    const orgId = req.orgId!;
    ...
    const ext = EXT_FROM_MIME[contentType] || "png";
    const key = makeKey(prefix, orgId, ext);

    // publicRead=true so logos/covers are viewable without a signed URL.
    // AWS SDK v3's s3-request-presigner hoists the ACL into the URL as a
    // query parameter, so the client does NOT need to send x-amz-acl in
    // the PUT request itself — just the Content-Type matching what was
    // signed here.
    const { uploadUrl } = await getPresignedUploadUrl(key, contentType, 900, true);

    res.json({ uploadUrl, key });
  } catch (error: any) {
    console.error(`${kind} presign error:`, error);
    res.status(500).json({ error: error.message || `Failed to prepare ${kind} upload` });
  }
}

/**
 * Shared handler body for logo/cover confirm. Validates the key is
 * scoped to the caller's org (preventing cross-org URL injection),
 * computes the permanent public URL, and persists it to orgBranding.
 */
async function handleConfirm(
  req: Request,
  res: Response,
  prefix: "org-logos" | "org-covers",
  column: "logoUrl" | "coverPhotoUrl",
  kind: "logo" | "cover"
) {
  try {
    const orgId = req.orgId!;
    ...
    // Key must start with "<prefix>/<sanitized-orgId>-" to prove it was
    // generated by this org's presign call. Prevents a malicious client
    // from confirming a key belonging to another org (which would
    // hijack their branding) or an arbitrary key pattern.
    const safeOrgId = orgId.replace(/[^a-zA-Z0-9_-]/g, "");
    const expectedPrefix = `${prefix}/${safeOrgId}-`;
    if (!key.startsWith(expectedPrefix)) {
      return res.status(403).json({ error: "Key does not belong to this org" });
    }

    const { isObjectStorageConfigured } = await import("./services/objectStorage");
    if (!isObjectStorageConfigured()) {
      return res.status(503).json({
        error: "File uploads are not configured on this server. Contact support.",
      });
    ...
    // Compute the permanent public URL the same way uploadPublicFile
    // does: CDN prefix (if configured) or bare Spaces hostname + "/" + key.
    ...
    // Persist to branding, upserting if no row exists yet.
    const [existing] = await db.select().from(orgBranding)
      .where(eq(orgBranding.orgId, orgId));

    if (existing) {
      await db.update(orgBranding)
        .set({ [column]: url, updatedAt: new Date() } as any)
        .where(eq(orgBranding.orgId, orgId));
    } else {
      await db.insert(orgBranding).values({ orgId, [column]: url } as any);
    }

    res.json({ url });
  } catch (error: any) {
    console.error(`${kind} confirm error:`, error);
    res.status(500).json({ error: error.message || `Failed to save ${kind}` });
  }
}

// POST /api/org/logo/presign — get a presigned PUT URL for logo upload
router.post("/org/logo/presign", requireAuth, requireOrg, async (req, res) => {
  await handlePresign(req, res, "org-logos", "logo");
});

// POST /api/org/logo/confirm — persist logo URL after successful PUT
router.post("/org/logo/confirm", requireAuth, requireOrg, async (req, res) => {
  await handleConfirm(req, res, "org-logos", "logoUrl", "logo");
});

// POST /api/org/cover/presign — get a presigned PUT URL for cover upload
router.post("/org/cover/presign", requireAuth, requireOrg, async (req, res) => {
  await handlePresign(req, res, "org-covers", "cover");
});

// POST /api/org/cover/confirm — persist cover URL after successful PUT
router.post("/org/cover/confirm", requireAuth, requireOrg, async (req, res) => {
  await handleConfirm(req, res, "org-covers", "coverPhotoUrl", "cover");
});
